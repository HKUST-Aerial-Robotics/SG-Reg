...
from model.utils.torch import build_dataloader
from model.dataset.ScenePairDataset import check_instance_points
...
def sgreg_collate_fn_stack_mode(data_dicts, 
    ...
    for scene_id in range(batch_size):
        ...
        if out_dict['points_dict']['lengths'][-1].sum() > point_limits:
            print('[WARNING] {} superpoints. Exhaustive Memory.'.format(out_dict['points_dict']['lengths'][-1].sum()))
